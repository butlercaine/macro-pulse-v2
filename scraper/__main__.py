...
Orchestrates the complete scraping pipeline:
    ...
    7. Optionally upload to Vercel Blob
"""
...
async def run_pipeline(
    ...
    if output_path:
        ...
        # Optionally upload to Vercel Blob
        if upload_blob:
            ...
            if upload_result.get("success"):
                ...
                errors.append(f"Blob upload failed: {upload_result.get('error')}")
                output.errors = errors
...
Examples:
  ...
  python -m scraper --blob                       # Upload to Vercel Blob
  python -m scraper --output data.json --blob     # Save + upload
  python -m scraper --help                        # Show this help
    ...
    parser.add_argument(
        "--blob",
        action="store_true",
        help="Upload output to Vercel Blob after scraping",
    )
    ...
    parser.add_argument(
        ...
        help="Vercel Blob path (default: scraper-data/latest.json)",
    )
